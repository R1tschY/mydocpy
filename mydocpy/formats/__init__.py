...
def load_formats():
    # type: () -> None
    """
    import all format modules and call ``register_formats(registry)`` on them
    (when a module has this function).
    """
    global registry
    ...
    for module_loader, name, ispkg in modules:
        if not ispkg:
            ...
            if hasattr(mdl, 'register_formats'):
                mdl.register_formats(registry)
